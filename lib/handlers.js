                        ...
                        if (!err && userData) {
                            ...
                            _data.update('users', phone, userData, function (err) {
                                if (!err) {
                                    ...
                                    console.log(err);
                                    callback(500, {'Error': 'Could not update the server'});
                                }
                            })
                        } else {
                            callback(403, {'Error': 'Missing required token in header, or token is invalid'});
...
handlers._tokens.post = function (data, callback) {
    ...
    if (phone && password) {
        ...
        _data.read('users', phone, function (err, userData) {
            if (!err && userData) {
                // Hash the sent password, and compare it to the password storied in the user object
                var hashedPassword = helpers.hash(password);
                ...
                _data.read('users', userPhone, function (err, userData) {
                    if (!err && userData) {
                        ...
                        if (userChecks.length < config.maxChecks) {
                            ...
                            callback(400, {'Eror': 'The user already has the maximum number of checks(' + config.maxChecks + ')'});
                        }
                    } else {
                        callback(403);
                    }
